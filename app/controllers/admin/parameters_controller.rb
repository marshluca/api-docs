class Admin::ParametersController < Admin::ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @parameter.save
        format.html { redirect_to project_doc_url(@doc.project, @doc), :notice => 'Parameter was successfully created.' }
  ...
  def update
    ...
    respond_to do |format|
      if @parameter.update_attributes(params[:parameter])
        format.html { redirect_to project_doc_url(@doc.project, @doc), :notice => 'Parameter was successfully updated.' }
        format.json { head :ok }
      else
