...
user = User.create! :name => 'admin', :email => 'admin@gmail.com', :password => 'secret', :password_confirmation => 'secret'
...
%w(wallpapers/recommend wallpapers/popular wallpapers/recent wallpapers/mine ringtones/recommend ringtones/popular ringtones/recent ringtones/mine).each do |i|
  Doc.create! :name => i, :format => 'json, xml', :http_method => 'Get', :project_id => project.id, :desc => '获取壁纸列表数据', :url => "http://ws.yfx365.com/v1/#{i}", :author => project.author, :category => project.categories.first
  puts
end

%w(ringtones/recommend ringtones/popular ringtones/recent ringtones/mine).each do |i|
  d = Doc.create! :name => i, :format => 'json, xml', :http_method => 'Get', :project_id => project.id, :desc => '获取铃声列表数据', :url => "http://ws.yfx365.com/v1/#{i}", :author => project.author, :category => project.categories[1]
